...
class SprintOrchestrator:
    ...
    def _maybe_capture_mike_failure_payload(self, story_id: str, failure_stage: str, response_text: str) -> Optional[str]:
        if os.getenv("CAPTURE_MIKE_FAILURE_PAYLOADS") not in {"1", "true", "TRUE", "yes", "YES"}:
            return None
            ...
            for pattern in key_patterns:
                for code_file in project_root.glob(pattern):
                    ...
                    try:
                        ...
                        exports = self._extract_exports_from_file(code_file)
...
"""
    ...
    async def _run_tests(self, project_root: Path, test_file: str = None) -> Dict:
        ...
        try:
            ...
            # Detect tech stack to determine test framework
            # tech_stack_details contains the dict; tech_stack is a string summary
            tech_stack_details = self.vision.get('tech_stack_details', {})
            backend = tech_stack_details.get('backend', '').lower()
            ...
            if 'nodejs' in backend or 'express' in backend:
                ...
                logger.info("Running Node.js native tests for Node.js project")
                ...
                logger.info("Running pytest for Python project")
...
"""
    ...
    def _extract_project_state(self, project_path: Path) -> Dict:
        ...
        state = {
            'database_schema': self._extract_database_schema(project_path),
            'api_endpoints': self._extract_api_endpoints(project_path),
            'file_structure': self._extract_file_structure(project_path),
    ...
    def _extract_database_schema(self, project_path: Path) -> str:
    ...
    def _extract_api_endpoints(self, project_path: Path) -> str:
    ...
    def _extract_exports_from_file(self, file_path: Path) -> List[str]:
    ...
    def _extract_file_structure(self, project_path: Path) -> str:
        ...
        try:
            ...
            for js_file in project_path.glob("src/**/*.js"):
                ...
                exports = self._extract_exports_from_file(js_file)
                file_info = {
                    'path': path_str,
                    'exports': exports
                }
    ...
    def _extract_code_patterns(self, project_path: Path) -> str:
